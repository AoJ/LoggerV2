...
(function($) {
	...
	/**
	 * foreach: customers, accordion: { collapsible: true, autoHeight: false }
	 * foreach: orders, accordion: {}
	 */
	ko.bindingHandlers.inspect = {
	    init: function(element, valueAccessor) {
	        var data = (valueAccessor ? valueAccessor() : {}) || {};
			var parsed;
			if(typeof data == 'string') try {
				parsed = JSON.parse(data);
			} catch(e) {  }
	        //let the content get build before initializing it
	        setTimeout(function() {
				console.log(parsed);
	           if(parsed) $(element).text('').inspect({data: parsed});
	        }, 0);
